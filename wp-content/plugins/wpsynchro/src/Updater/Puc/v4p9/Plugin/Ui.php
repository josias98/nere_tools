	...
	class WPSynchroPuc_v4p9_Plugin_Ui {
		...
		public function addViewDetailsLink($pluginMeta, $pluginFile, $pluginData = []) {
			if ( $this->isMyPluginFile($pluginFile) && !isset($pluginData['slug']) ) {
				$linkText = apply_filters($this->updateChecker->getUniqueName('view_details_link'), 'View details');
				if ( !empty($linkText) ) {
					...
					$viewDetailsLink = sprintf('<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
						...
						esc_attr('More information about WP Synchro'),
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['WPSynchroPuc_update_check_result'], $_GET['WPSynchroPuc_slug']) && ($_GET['WPSynchroPuc_slug'] == $this->updateChecker->slug) ) {
				...
				if ( $status == 'no_update' ) {
					$message = 'The WP Synchro plugin is up to date.';
				} else if ( $status == 'update_available' ) {
					$message = 'A new version of the WP Synchro plugin is available.';
				} else if ( $status === 'error' ) {
					$message = 'Could not determine if updates are available for WP Synchro.';
