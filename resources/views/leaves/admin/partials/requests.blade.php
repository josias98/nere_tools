<section class="nc-panel" id="requests">
    ...
    @forelse ($requests as $leave)<tr><td>{{ $leave->employee?->name() }}</td><td>{{ $leave->leaveType?->name }}</td><td>{{ $leave->periodLabel() }}</td><td>{{ $leave->durationLabel() }}</td><td>{{ $leave->effective_return_at?->format('d/m/Y H:i') ?? 'Historique' }}</td><td><span class="leave-status is-{{ $leave->status }}">{{ $leave->statusLabel() }}</span></td><td><a class="nc-link" href="{{ route('leaves.show', $leave->uuid) }}">Ouvrir</a></td></tr>@empty<tr><td colspan="7"><div class="nc-empty">Aucune demande ne correspond aux filtres.</div></td></tr>@endforelse
