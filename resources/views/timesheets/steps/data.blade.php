<section aria-labelledby="step-title" class="ts-step-panel">
    <div class="ts-step-heading"><span>Constitution du lot</span><h2 id="step-title">{{ $method === 'csv' ? 'Déposez votre clé de répartition' : 'Qui doit figurer dans le lot&nbsp;?' }}</h2><p>{{ $method === 'csv' ? 'L’analyse démarre uniquement quand vous le décidez.' : count($rows).' collaborateur(s) sélectionné(s).' }}</p></div>
    @if ($method === 'csv')
    ...
    @else
        ...
        <div class="ts-selected-list">@forelse($rows as $index => $row)<div wire:key="employee-{{ $row['employee_id'] }}" wire:transition.scale.origin.top.duration.160ms><span class="ts-person-mark">{{ mb_strtoupper(mb_substr($row['first_name'], 0, 1).mb_substr($row['last_name'], 0, 1)) }}</span><span><strong>{{ $row['first_name'] }} {{ $row['last_name'] }}</strong><small>{{ $row['entity_name'] }}</small></span><button type="button" class="nc-ghost" wire:click="removeRow({{ $index }})" aria-label="Retirer {{ $row['first_name'] }} {{ $row['last_name'] }}"><i data-lucide="x" aria-hidden="true"></i></button></div>@empty<div class="ts-empty-state"><i data-lucide="user-plus" aria-hidden="true"></i><strong>Le lot est encore vide</strong><span>Commencez par saisir un nom dans le champ de recherche.</span></div>@endforelse</div>
